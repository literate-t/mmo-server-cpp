...
public:
	static void Init()
	{
		...
		g_packet_handler[PKT_C_ENTER_GAME] = [](SharedPacketSession& session, BYTE* buffer, int32 length) { return ParsePacket <Protocol::C_ENTER_GAME>(Handle_C_ENTER_GAME, session, buffer, length); };
		g_packet_handler[PKT_C_CHAT] = [](SharedPacketSession& session, BYTE* buffer, int32 length) { return ParsePacket <Protocol::C_CHAT>(Handle_C_CHAT, session, buffer, length); };		
	}
	...
	static SharedSendBuffer MakeSendBuffer(Protocol::S_CHAT& pkt) { return MakeSendBuffer(pkt, PKT_S_CHAT); }	
